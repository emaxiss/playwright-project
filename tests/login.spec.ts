...
test.describe("Login", () => {
    ...
    test("should login with valid credentials", async ({ loginPage, kanbanPage }) => {
        await loginPage.login(adminUser.username, adminUser.password);
        await kanbanPage.page.waitForURL(kanbanPage.url);
        await expect(kanbanPage.header.logoutButton).toBeVisible();
    });

    test("should not allow login with invalid username", async ({ loginPage }) => {
    ...
    test("should not allow login with invalid password", async ({ loginPage }) => {
